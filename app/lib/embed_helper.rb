...
module EmbedHelper
  ...
  def scale_preview_sizes(
    entry,
    ui_minwidth: UI_MIN_WIDTH, ui_minheight: UI_MIN_HEIGHT,
    maxwidth: nil, maxheight: nil
  )
    type = entry.media_type.to_sym
    default_width = UI_DEFAULT_WIDTH
    default_height = UI_DEFAULT_HEIGHT

    # NOTE: source sizes ignore for now
    # case type
    # when :video
    ...
    max_height = maxheight.nil? ? source_height : [maxheight.to_i, source_height].min
    ...
    # dont go smaller than the UI supports
    {
      ...
      height: [(source_height * scale), ui_minheight].max.to_i
    }
  end
