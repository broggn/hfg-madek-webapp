...
feature 'Media Entry - Managing Temporary URLs' do
  ...
  scenario 'Revoking' do
    confidential_link = create :confidential_link, user: @user, resource: @entry

    sign_in_as @user.login
    visit confidential_links_media_entry_path(@entry)

    row = find('.ui-resources-holder table tr', text: confidential_link.token)
    ...
    expect(current_path).to eq confidential_links_media_entry_path(@entry)
    confidential_link.reload
    expect(confidential_link.revoked).to be true

    expect(displayed_ui(revoked: true)).to eq [
      [
        confidential_link.token,
        confidential_link.description,
        I18n.t(:confidential_links_list_no_expiry),
        I18n.t(:confidential_links_list_show_url),
        ''
      ]
